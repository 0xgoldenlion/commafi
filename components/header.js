...
export default function Header() {
  ...
  const renderConnectWallet = () => {
    if (!accountQuery.data?.address) {
      return (
        <button
          className="text-lg font-medium rounded-md px-5 py-3 bg-gradient-to-r from-black to-black hover:from-stone-900 hover:to-stone-900"
          onClick={() => {
      ...
      return (
        <div className="flex flex-wrap gap-5 justify-center items-center">
          ...
          <button
            onClick={disconnect}
            className="text-lg font-medium rounded-md px-5 py-3 bg-gradient-to-r bg-gradient-to-r from-black to-black hover:from-stone-900 hover:to-stone-900"
          >
            Disconnect
          </button>
        </div>
  ...
  return (
    <header className="flex flex-wrap justify-between p-5 mb-10">
      <Link href="/">
        <a className="text-xl md:mb-auto mb-5 font-bold text-transparent bg-clip-text bg-white">
          Comma
        </a>
      </Link>
